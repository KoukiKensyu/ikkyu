...
<tr><td>{{$reserve->name}}</td><td>{{$reserve->reserved_date}}</td><td>{{$reserve->checkin_date}}</td><td>{{$reserve->checkout_date}}</td><td>{{$reserve->price * $reserve->day->d * $reserve->rooms}}円</td></tr>
@endforeach
</table>
未宿泊の予定はキャンセルします<br>
@endif
